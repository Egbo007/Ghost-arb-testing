...
async function scan(allPools) {
  ...
  for (const [pairName, pools] of Object.entries(groups)) {
    ...
    // FIX 2: Nested loop — check ALL buy/sell combinations, not just min/max.
    // Example: V3 0.01% buy → V2 sell might beat V3 1% buy → vAMM sell after fees.
    // FIX 4: This also removes the broken "alt buy pool" substitution code.
    let bestOpp = null;

    for (const buyPool of sanePools) {
      for (const sellPool of sanePools) {
        if (buyPool.address === sellPool.address) continue; // can't trade with yourself
        if (buyPool.price >= sellPool.price) continue;      // must buy cheap, sell dear

        const gapPct = ((sellPool.price - buyPool.price) / buyPool.price) * 100;
        if (gapPct < MIN_GAP_PERCENT) continue;
        if (gapPct > 50) continue; // bad price data

        // FIX 3 (fee check): total fees must be less than the gap.
        // buy fee + sell fee + flash loan fee >= gap → impossible profit, skip.
        const totalFeePct = (buyPool.fee * 100) + (sellPool.fee * 100) + FLASH_FEE_PCT;
        if (totalFeePct >= gapPct) {
          // Only log if this is the best raw gap (reduces noise)
          if (!bestOpp) {
            console.log(`  ⛔ ${pairName}: Best combo fees (${totalFeePct.toFixed(2)}%) >= gap (${gapPct.toFixed(3)}%) — mathematically impossible`);
          }
          continue;
        }
    ...
    if (!bestOpp) {
      // Log best raw gap for visibility even when no profit found
      const sorted = [...sanePools].sort((a,b) => a.price - b.price);
      const rawGap = ((sorted[sorted.length-1].price - sorted[0].price) / sorted[0].price * 100).toFixed(3);
      console.log(`  ❌ ${pairName}: Best raw gap ${rawGap}% — no profitable combo after fees`);
      continue;
    }
...
async function logToSheets(data) {
  if (!SHEETS_WEBHOOK_URL) return; // skip if not configured
  try {
    ...
    console.log(`  📊 Sheets log failed (non-critical): ${e.message}`);
  }
}

async function main() {
  console.log('╔══════════════════════════════════════╗');
  console.log('║   Ghost Arb Monitor v1.11             ║');
  ...
  await sendTelegram(`🤖 <b>Ghost Arb Monitor v1.11 LIVE</b>
