...
function filterSanePrices(pools) {
  ...
  return pools.filter(p => {
    ...
    const sane = ratio > 0.5 && ratio < 2.0; // tightened: removes sAMM bad prices
    ...
    for (const factory of FACTORIES) {
      try {
        ...
        if (factory.type === 'AERO') {
          ...
          // sAMM only for stablecoin-stablecoin pairs. For ETH/BTC pairs it gives wrong prices.
          const isStablePair = stables.includes(pair.tokenA.symbol) && stables.includes(pair.tokenB.symbol);
          for (const stable of [false, ...(isStablePair ? [true] : [])]) {
...
async function main() {
  console.log('╔══════════════════════════════════════╗');
  console.log('║   Ghost Arb Monitor v4.6             ║');
  console.log('║   sAMM Fix + Tight Price Filter     ║');
  console.log('╚══════════════════════════════════════╝');
  ...
  await sendTelegram(`🤖 <b>Ghost Arb Monitor v4.6 LIVE</b>
