...
function filterSanePrices(pools) {
  ...
  return pools.filter(p => {
    ...
    const sane = ratio > 0.05 && ratio < 20;
...
async function getV3PoolData(pool) {
  try {
    ...
    const [slot0Data, token0Addr, token1Addr, liquidityRaw] = await Promise.all([
      poolContract.slot0(),
      poolContract.token0(),
      poolContract.token1(),
    ...
    // ── TOTAL TVL via balanceOf — used for FILTER DECISION ───────────────
    // Fault 1 fix: active liquidity is often tiny even in large pools.
    // Use full token balance to decide "is this pool worth watching?"
    ...
    const [bal0, bal1] = await Promise.all([
      t0.balanceOf(pool.address),
      t1.balanceOf(pool.address),
    ]);
    let tvlX, tvlY;
    if (isTokenAToken0) {
      tvlX = parseFloat(ethers.formatUnits(bal0, pool.tokenA.decimals));
      tvlY = parseFloat(ethers.formatUnits(bal1, pool.tokenB.decimals));
    } else {
      tvlX = parseFloat(ethers.formatUnits(bal1, pool.tokenA.decimals));
      tvlY = parseFloat(ethers.formatUnits(bal0, pool.tokenB.decimals));
    }
    const liquidityUSD = computeLiquidityUSD(pool, tvlX, tvlY); // TVL for filtering

    // ── ACTIVE RESERVES via L — used for SLIPPAGE MATH only ──────────────
    // Fault 3 fix: if L=0 (price at empty tick), fall back to 2% of TVL
    // instead of dropping pool. Pool stays visible, tiny trade sizes will
    // be the only ones that pass the profit check (honest behaviour).
    ...
    let activeX, activeY;
    if (L > 0) {
      const x0_raw = L / sqrtP_raw;
      const y0_raw = L * sqrtP_raw;
      if (isTokenAToken0) {
    ...
    // Fallback if active reserves are invalid or zero
    if (!activeX || !activeY || activeX <= 0 || activeY <= 0 || !isFinite(activeX) || !isFinite(activeY)) {
      activeX = tvlX * 0.02; // conservative 2% of TVL
      activeY = tvlY * 0.02;
    }
    ...
    return {
      ...pool,
      x: activeX,       // Active reserves → slippage simulation
      y: activeY,
      k: activeX * activeY,
      tvlX, tvlY,       // Total balance → display context
      price,
      liquidityUSD,     // Based on TVL → filter decision
      activeOnly: true,
    };
  } catch (e) { return null; }
}
...
async function scan(allPools) {
  ...
  // ── FAULT 2 FIX: Update ETH/BTC price FIRST on raw unfiltered data ──────
  // Previously: if WETH/USDC V3 pools got dropped by Fault 1, ETH price
  // never refreshed → meme coin pairs computed liquidityUSD = 0 → also dropped.
  // Now: price update runs on ALL pool results before any filter is applied.
...
async function main() {
  console.log('╔══════════════════════════════════════╗');
  console.log('║   Ghost Arb Monitor v4.4             ║');
  console.log('║   TVL Filter + Active Slippage Fix   ║');
  console.log('╚══════════════════════════════════════╝');
  ...
  await sendTelegram(`🤖 <b>Ghost Arb Monitor v4.4 LIVE</b>

✅ TVL (balanceOf) used for pool filtering
✅ Active liquidity (L) used for slippage math only
✅ ETH/BTC price updates before any filtering (cascade fix)
✅ L=0 fallback: 2% TVL estimate instead of dropping pool
✅ Batched RPC calls (25 at a time, no rate limit drops)
✅ 12 pairs across ${FACTORIES.length} DEXes
