...
async function getV3PoolData(pool) {
  try {
    ...
    if (L === 0) return null; // No active liquidity in this tick range
...
async function scan(allPools) {
  ...
  // Batched RPC calls — 25 at a time with 300ms delay between batches
  // Prevents Alchemy free tier rate limits (was causing "0 pools pass" on alternate scans)
  const poolDataRaw = [];
  const BATCH_SIZE = 25;
  for (let i = 0; i < allPools.length; i += BATCH_SIZE) {
    ...
    if (i + BATCH_SIZE < allPools.length) await sleep(300);
  }
  ...
  const belowThreshold = poolDataRaw.filter(p => p !== null).length - poolData.length;
  console.log(`  Active pools: ${poolData.length} | RPC failed: ${nullCount} | Below threshold: ${belowThreshold}`);
...
async function main() {
  console.log('╔══════════════════════════════════════╗');
  console.log('║   Ghost Arb Monitor v4.3             ║');
  console.log('║   Batched RPC + Lower Thresholds     ║');
  console.log('╚══════════════════════════════════════╝');
  ...
  await sendTelegram(`🤖 <b>Ghost Arb Monitor v4.3 LIVE</b>

✅ Batched RPC calls (no more rate limit drops)
✅ Min liquidity lowered: $10k core, $20k meme
✅ Active V3 liquidity (liquidity() not balanceOf)
✅ 12 pairs: 4 stables + cbETH + 3 BTC + 3 meme
✅ Live ETH + BTC price tracking
