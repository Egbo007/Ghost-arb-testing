...
// v2.1 FIX: BigInt precision throughout — no more Number(sqrtPriceX96).
//
// OLD CODE (v2.0):
...
//
// WHY IT WAS WRONG:
//   sqrtPriceX96 is a uint160 (up to 2^160). JS Number only has 53 bits of
//   precision (MAX_SAFE_INTEGER = 2^53 ≈ 9e15). Converting a 160-bit value to
//   Number silently loses precision, introducing pricing errors — fatal when
//   arb margins are 0.3%.
//
// NEW CODE: All squaring done in BigInt. Only convert to Number at the very end
// after scaling down to a safe range.
//
// MATH: rawPrice = sqrtPriceX96^2 / 2^192
//   Step 1: sqrtP = sqrtPriceX96 / 2^48  (at most 2^112 — safe to square)
//   Step 2: rawScaled = sqrtP^2 * 1e9 / 2^96
//   Step 3: rawPrice = Number(rawScaled) / 1e9
//   Net:    rawScaled/1e9 = sqrtP^2/2^96 = sqrtPriceX96^2/(2^48*2^48*2^96)
//                         = sqrtPriceX96^2/2^192  ✓ same formula, BigInt safe
function sqrtPriceX96ToHumanPrice(sqrtPriceX96BN, isTokenAToken0, tokenADec, tokenBDec) {
  ...
  const sqrtP     = sqrtPriceX96BN / (2n ** 48n); // brings into 2^112 range before squaring
  const PREC      = 10n ** 9n;
  const rawScaled = (sqrtP * sqrtP * PREC) / (2n ** 96n);
  const rawPrice  = Number(rawScaled) / Number(PREC);

  if (!rawPrice || rawPrice === 0 || !isFinite(rawPrice)) return null;

  const decimalFactor = Math.pow(10, tokenADec - tokenBDec);
  return isTokenAToken0
    ? rawPrice * decimalFactor
    : (1 / rawPrice) * decimalFactor;
}
