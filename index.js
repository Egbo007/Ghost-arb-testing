...
async function getV3PoolData(pool) {
  try {
    ...
    // Fetch total TVL via balanceOf (for display alongside active)
    let tvlUSD = liquidityUSD; // fallback = same as active-based
    try {
      const t0c = new ethers.Contract(token0Addr, ERC20_ABI, provider);
      const t1Addr_actual = isTokenAToken0 ? pool.tokenB.address : pool.tokenA.address;
      const t1c = new ethers.Contract(t1Addr_actual, ERC20_ABI, provider);
      const [b0, b1] = await Promise.all([t0c.balanceOf(pool.address), t1c.balanceOf(pool.address)]);
      const tvlX = isTokenAToken0
        ? parseFloat(ethers.formatUnits(b0, pool.tokenA.decimals))
        : parseFloat(ethers.formatUnits(b1, pool.tokenA.decimals));
      const tvlY = isTokenAToken0
        ? parseFloat(ethers.formatUnits(b1, pool.tokenB.decimals))
        : parseFloat(ethers.formatUnits(b0, pool.tokenB.decimals));
      tvlUSD = computeLiquidityUSD(pool, tvlX, tvlY);
    } catch { /* keep fallback */ }
    ...
    return {
      ...
      k: activeX * activeY,
      price,
      liquidityUSD: tvlUSD,    // TVL — used for filter threshold
      activeLiqUSD: liquidityUSD, // Active only — shown in logs/alerts
      isV3: true,
    };
  } catch (e) { return null; }
}
...
async function scan(allPools) {
  ...
  for (const [pairName, pools] of Object.entries(groups)) {
    ...
    const prices = sorted.map(p => {
      ...
      const label = p.isV3
        ...
        : p.dex.split(' ')[0];
      const liqStr = p.isV3
        ? `TVL:${fmt(p.liquidityUSD)}|Act:${fmt(p.activeLiqUSD || 0)}`
        : `Liq:${fmt(p.liquidityUSD)}`;
      return `${label}=$${p.price.toFixed(2)}[${liqStr}]`;
    }).join(' | ');
  ...
  for (const [pairName, pools] of Object.entries(groups)) {
    ...
    // KEY FIX: if buy pool fee alone >= gap, profit is impossible regardless of size.
    // V3 1% pool with 0.8% gap = guaranteed loss. Skip before wasting simulation.
    if (buyPool.fee * 100 >= gapPct) {
      console.log(`  ⛔ ${pairName}: Buy fee (${(buyPool.fee*100).toFixed(2)}%) eats entire gap (${gapPct.toFixed(3)}%) — buy pool unusable`);
      // Try next cheapest pool as buy instead
      if (sanePools.length > 2) {
        const altBuy = sanePools[1]; // second cheapest
        const altGap = ((sellPool.price - altBuy.price) / altBuy.price) * 100;
        if (altGap >= MIN_GAP_PERCENT && altBuy.fee * 100 < altGap) {
          console.log(`  🔄 ${pairName}: Trying alt buy: ${altBuy.dex} @ $${altBuy.price.toFixed(2)} — gap=${altGap.toFixed(3)}%`);
          sanePools[0] = altBuy; // swap in alt buy pool for simulation below
        } else {
          continue;
        }
      } else {
...
🔻 <b>BUY on</b> ${opp.buyPool.dex}
   ...
   TVL: $${Math.round(opp.buyPool.liquidityUSD).toLocaleString()}${opp.buyPool.isV3 ? ` | Active: $${Math.round(opp.buyPool.activeLiqUSD||0).toLocaleString()}` : ''}
...
async function main() {
  console.log('╔══════════════════════════════════════╗');
  console.log('║   Ghost Arb Monitor v1.8             ║');
  console.log('║   Fee Filter + Proper Versioning      ║');
  console.log('╚══════════════════════════════════════╝');
  ...
  await sendTelegram(`🤖 <b>Ghost Arb Monitor v1.8 LIVE</b>

✅ Fee pre-check: skips unprofitable buy pools before simulation
